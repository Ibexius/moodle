...
class simplecertificate {
    ...
    protected function create_pdf(stdClass $issuecert, $pdf = null, $isbulk = false) {
       ...
       // $pdf->SetXY($this->get_instance()->certificatetextx, $this->get_instance()->certificatetexty);
		...
		$htmlCertificateText = '
		<div style="color:#00245D; font-size:2em;">
			<br>
			<h4 style="text-align: center; color:#4D91F7; font-size:28px; line-height:0px;"><b>CERTIFICATE</b><br></h4>

			<p style="font-size:20px; line-height:0px;">This certificate is awarded to</p>
			<p style="font-size:25px; text-transform:uppercase; line-height:0px;"><b><i>{USERNAME}</i></b><br></p>

			<p style="font-size:20px; line-height:0px;">for completion of the course</p>
			<p style="font-size:25px; text-transform:uppercase; line-height:0px;"><b><i>{COURSENAME}</i></b><br></p>		
			
			<p style="font-size:20px; line-height:0px;">on the day of</p>
			<p style="font-size:22px; line-height:0px;"><b>{DATE}</b></p>
			
</div>
';
		$htmlCertificateTextReferences = '
		<div style="text-align:left; font-size:10px;">
		<h3 style="line-height:1px;">
			References:
		</h3>'. $this->get_certificate_text($issuecert, $this->get_instance()->certificatetext).'</div>';

		$htmlCertificateTextFooter = '<p style="color:#00245D;"><a style="color:#00245D !important; text-decoration:none;" href="http://ael.aero">www.ael.aero</a> &nbsp;&nbsp; email:<a  style="color:#00245D !important; text-decoration:none;" href="mailto:info@ael.aero"> info@ael.aero</a> &nbsp;&nbsp; company reg. nr.: DK 33 25 30 87</p>';
		
		$pdf->SetFont('helvetica');
		
		
		$imageUrl = new moodle_url("$CFG->wwwroot/mod/simplecertificate/certificate-images/Certificate1.png");
		//(string)$myVar
		$pdf->Image((string)$imageUrl,0, 0, 297, 210, '', '', '', false, 300, '', false, false, 0);
		//$pdf->writeHTML($htmlCertificateText,false,true,false,false,'C') ;
		
		$pdf->writeHTMLCell(0, 0, '', 30, $this->get_certificate_text($issuecert, $htmlCertificateText), 0, 0, 0, 
                            true, 'C');
		if (!$this->get_certificate_text($issuecert, $this->get_instance()->certificatetext) == '' || !$this->get_certificate_text($issuecert, $this->get_instance()->certificatetext) == null)
		{
			$pdf->writeHTMLCell(0, 0, 40, 140, $htmlCertificateTextReferences, 0, 0, 0, true, 'C');
		}
		
		$pdf->writeHTMLCell(0, 0, '', 197, $htmlCertificateTextFooter, 0, 0, 0, 
                            true, 'C');
    ...
    protected function print_qrcode($pdf, $code) {
        ...
        $pdf->write2DBarcode($codeurl->out(false), 'QRCODE,M', 0, 164, 30, 30, 
                            $style, 'N');
        $pdf->SetXY(0, 164 + 30);
        $pdf->SetFillColor(255, 255, 255);
        //$pdf->Cell(30, 0, $code, 'LRB', 0, 'C', true, '', 1);
		$pdf->writeHTMLCell(33,0,-1,164+28, '<span style="font-size:5px; line-height:1px;">Certificate Unique Nr.</span>', 0, 0,false,'', 'C');
		
		$pdf->writeHTMLCell(33,0,-1,164+30, '<span style="font-size:5px; line-height:9px;letter-spacing: -0.3px;">' . $code . '</span>', 0, 0,false,'', 'C');
