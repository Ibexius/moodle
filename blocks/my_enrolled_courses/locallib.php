...
function block_my_enrolled_courses_visible_in_block() {
    ...
    $html .= html_writer::start_tag('div', array('class' => 'row','id' => 'course_list_in_block'));
    if (! empty($coursesinorder)) {
    		$coursesinorderstr = implode(', ', $coursesinorder);
        $courses = $DB->get_records_sql('SELECT id, fullname, summary FROM {course} WHERE id IN (' . $coursesinorderstr . ')');
        foreach ($coursesinorder as $id) {
			...
			$contentImage = '';
			$courseId=$id;
			$courseimage = get_courses("all");
			
			foreach($courseimage as $key=>$ci){
				if($ci->id == $courseId)
				{
					$title = $ci->fullname;
					...
					if ($ci instanceof stdClass) {
						...
						$courseimage = new course_in_list($ci);
						//print_r("works");
					}
				   
					$fileurl = $CFG->wwwroot."/blocks/my_enrolled_courses/coursenoimage.jpg";
					...
					$contentImage .= '<img class="img-100" src="'.$fileurl.'" alt="'.$title.'">';     
				}								   
			}
			$coursethumbnail = html_writer::link($url,$contentImage);
			
			
            $content .= "$courseicon $anchor $colapsible $coursethumbnail";
            ...
            $html .= html_writer::tag('div', $content, array('class' => 'course_list_item_in_block col-sm-3'));
